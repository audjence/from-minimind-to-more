...
> 协变量偏移（Covariate Shift）：由于前一层参数的不断更新，导致后一层输入数据的分布（如均值、方差）在训练过程中持续变化。这会迫使高层网络不断适应新的分布，降低收敛速度，并可能使激活值落入饱和区导致梯度消失。归一化技术通过强制稳定每层输入的分布来缓解这一问题，使损失曲面更平滑，允许使用更大的学习率。
...
> **层归一化不影响 RoPE**，两者独立且互补：
> - **LayerNorm** 对每个 token 的特征向量进行缩放平移，不改变 token 之间的相对关系。
> - **RoPE** 通过旋转矩阵对 Query 和 Key 施加位置编码，旋转是正交变换，不改变向量的模长。
> 
> **主流顺序**（Pre-Norm + RoPE）：  
> 
> ```
> x = LayerNorm(x)               # 或 RMSNorm
> q, k, v = linear_qkv(x)
> q = apply_rotary_pos_emb(q)    # 对 Q/K 施加 RoPE
> k = apply_rotary_pos_emb(k)
> attn = softmax(q @ k.T / sqrt(d))
> ```
> 
> 所有主流模型（Llama、Mistral、Qwen、DeepSeek）均采用此设计。LayerNorm 的归一化不会破坏 RoPE 的相对位置性质，因为旋转后的内积仍然仅依赖相对距离，且模型可以自适应调整缩放因子。
...
> **梯度与权重模长成反比的简化推导**
> 考虑一个全连接层后接 LN：
> $$\mathbf{y} = \mathrm{LN}(W\mathbf{h})$$。
> 设 $$W' = \lambda W$$（权重放大 $$\lambda$$ 倍），则 $$W'\mathbf{h} = \lambda (W\mathbf{h})$$。
> 由重缩放不变性，输出 $$\mathbf{y}$$ 保持不变。
> 
> 对 $$\lambda$$ 求导（链式法则）：
> 
> $$0 = \frac{dL}{d\lambda} = \frac{\partial L}{\partial W'} \cdot \frac{dW'}{d\lambda} = \frac{\partial L}{\partial W'} \cdot W$$
> 
> 这并不直接给出反比关系，更直接地，记 $$\mathbf{u} = W\mathbf{h}$$，则 $$\mathbf{u}' = \lambda \mathbf{u}$$。
> 由于 $$\mathbf{y}$$ 不变，
> $$\frac{\partial L}{\partial \mathbf{u}'} = \frac{1}{\lambda} \frac{\partial L}{\partial \mathbf{u}}$$。于是：
> 
> $$\frac{\partial L}{\partial W'} = \frac{\partial L}{\partial \mathbf{u}'} \cdot \mathbf{h}^T = \frac{1}{\lambda} \frac{\partial L}{\partial \mathbf{u}} \cdot \mathbf{h}^T = \frac{1}{\lambda} \frac{\partial L}{\partial W}$$
> 
> **结论**：权重放大 $$\lambda$$ 倍时，梯度缩小为原来的 $$1/\lambda$$ —— 梯度大小与权重模长成反比。这提供了隐式的正则化，防止权重无限增长。
...
> **作用**：  
> 初始化时将子层中部分权重乘以一个小系数 \(\beta\)（如 \(\beta = (8N)^{-1/4}\)），使子层初始输出幅度很小。配合大的 \(\alpha\)，训练初期模型近似恒等映射，保证梯度稳定；随着训练，权重自适应增长。
> 
> **为何只对 \(W_V, W_O\) 而不对 \(W_Q, W_K\) 缩放？**
> 
> - **\(W_V\) 和 \(W_O\)** 直接影响子层输出 \(G_l\) 的幅度。缩放它们可以控制子层对残差路径的贡献大小，与放大 \(\alpha\) 形成互补。
> - **\(W_Q, W_K\)** 用于计算注意力分数 \(QK^T\)。若缩放它们，会改变注意力分布的锐度（softmax 的温度），可能破坏模型对相对重要性的学习。保持标准初始化（如 Xavier）可维持合理分数范围，确保注意力机制正常。
> 
> 同理，FFN 的所有权重也被缩放，因为其输出同样贡献到残差。**原则**：只缩放那些直接决定子层输出幅度的矩阵，而不影响注意力分数的计算。
